...
type MoneyRepository interface {
	...
	// GetOnSaleMoneys(ctx context.Context) ([]domain.Money, error)
...
func (r *MoneyDBRepository) GetMoney(ctx context.Context, id int32) (domain.Money, error) {
	row := r.QueryRowContext(ctx, "SELECT * FROM money2 WHERE id = ?", id)

	var item domain.Money
	return item, row.Scan(&item.ID, &item.Name, &item.Price, &item.CategoryID, &item.UserID, &item.CreatedAt, &item.UpdatedAt)
}
