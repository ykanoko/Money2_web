import { Anchor, Box, Button, Group, NumberInput, PasswordInput, SegmentedControl, Stack, Table, TextInput, Title } from "@mantine/core";
import { useForm } from "@mantine/form";
import { AddIncomeRecordRequest, GetMoneyRecord, POSTRegisterRequest, addIncomeRecord, addPairExpenseRecord, getMoneyRecord, registerUser } from "../api/api";
...
export default function AddMoneyRecordPage() {
  ...
  function handleSubmit(values: Form){
    ...
    if (values.payType === '1'){
      addIncomeRecord({amount:values.amount, user_id:values.user_id}, token).then(res => {
        navigate('/money_record')
      })
    }else if (values.payType === '2'){
      addPairExpenseRecord({amount:values.amount, user_id:values.user_id}, token).then(res => {
        navigate('/money_record')
      })
    } 
  }
  ...
  if (!user1 || !user2){
    return   <Stack  maw={300} mx="auto" mt={"md"}>You are not logged in
  ...
  return (
    ...
    <Title order={2} mt={"md"}>Add Money Record</Title>
